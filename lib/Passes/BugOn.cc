...
static cl::opt<bool> ShowTrueOpt("show-bugon-true",
                                 cl::desc("Show always true bug conditions"),
                                 cl::init(true));

Function *getBugOn(const Module *M) { return M->getFunction(OPT_BUGON); }
...
Function *getOrInsertBugOn(Module *M) {
    ...
    Function *F =
        cast<Function>(M->getOrInsertFunction(OPT_BUGON, T).getCallee());
    F->setDoesNotThrow();
    return F;
}
...
Value *BugOnPass::createIsZero(Value *V) { return Builder->CreateIsNull(V); }
...
Value *BugOnPass::createIsSDivWrap(Value *L, Value *R) {
    ...
    return createAnd(Builder->CreateICmpEQ(L, SMin),
...
Value *BugOnPass::createPointerEQ(Value *V0, Value *V1) {
    if (V0 == V1)
        return Builder->getTrue();
    return Builder->CreateICmpEQ(V0,
                                 Builder->CreatePointerCast(V1, V0->getType()));
}
