...
bool BugOnIntPass::runOnInstruction(Instruction *I) {
    ...
    case Instruction::Add:
        if (BO->hasNoSignedWrap())
            Changed |=
                insert(createIsSAddWrap(L, R), "signed addition overflow");
        if (BO->hasNoUnsignedWrap())
            Changed |=
                insert(createIsUAddWrap(L, R), "unsigned addition overflow");
        break;
    case Instruction::Sub:
        if (BO->hasNoSignedWrap())
            Changed |=
                insert(createIsSSubWrap(L, R), "signed subtraction overflow");
        if (BO->hasNoUnsignedWrap())
            Changed |=
                insert(createIsUSubWrap(L, R), "unsigned subtraction overflow");
        break;
    case Instruction::Mul:
        if (BO->hasNoSignedWrap())
            Changed |= insert(createIsSMulWrap(L, R),
                              "signed multiplication overflow");
        if (BO->hasNoUnsignedWrap())
            Changed |= insert(createIsUMulWrap(L, R),
    ...
    case Instruction::Shl:
        ...
        if (BO->hasNoSignedWrap()) {
            ...
            Changed |= insert(createIsSMulWrap(L, Power),
                              "signed shift left overflow");
        }
        if (BO->hasNoUnsignedWrap()) {
            Value *Power = Builder->CreateShl(ConstantInt::get(T, 1), R);
            Changed |= insert(createIsUMulWrap(L, Power),
                              "unsigned shift left overflow");
        }
        break;
...
bool BugOnIntPass::visitShiftOperator(IntegerType *T, Value *R,
                                      const char *Bug) {
